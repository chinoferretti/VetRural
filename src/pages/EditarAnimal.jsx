...
const RAZAS            = ['Aberdeen Angus', 'Hereford', 'Limousin', 'Shorthorn', 'Criollo', 'Bradford', 'Brangus', 'Charolais', 'Otra'];
...
export default function EditarAnimal() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      if (!animal) { setCargando(false); return; }
      ...
      // Agregar lote del animal si no está en la lista
      if (animal.lote && !LOTES_DEFAULT.includes(animal.lote)) {
        setLotes(prev => [...prev, animal.lote]);
      }
      setCargando(false);
    }, 400);
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      setGuardando(false);
      setExito(true);
      // replace: true para que el botón "atrás" del browser no vuelva al formulario

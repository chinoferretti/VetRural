...
import { getAnimalById, getHistorialAnimal, actualizarAnimal } from '../api/animalesApi';
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        ...
        style={{ ...inputSty, backgroundColor: '#F9FAFB', border: '1px solid #E5E7EB', color: '#6B7280',
          fontFamily: label === 'Caravana electrónica' ? 'monospace' : undefined, fontSize: '1rem' }}
      >
        {valor}
      </div>
...
export default function EditarAnimal() {
  ...
  useEffect(() => {
    ...
    // 1. Obtener datos básicos del bovino para conseguir la caravana
    getAnimalById(id)
      .then(bovino => {
        if (bovino.lote) setLotes([bovino.lote]);
        // 2. Obtener historial clínico usando la caravana
        return getHistorialAnimal(bovino.caravana).catch(() => bovino);
      })
      .then(animal => {
        setForm({
          ...
          tipo:             animal.tipo               || '',
          pelaje:           '',
          boqueo_dientes:   animal.boqueo?.cantidadDientes ?? '',
          boqueo_deterioro: animal.boqueo?.deterioro       ?? '',
          boqueo_dentadura: animal.boqueo?.tipoDentadura   ?? '',
          peso:             animal.peso               ?? '',
          tacto_situacion:  animal.tacto?.situacion   ?? '',
          tacto_periodo:    animal.tacto?.periodo     ?? '',
          vac_aftosa:       animal.vacunacion?.aftosa      ?? '',
          vac_brucelosis:   animal.vacunacion?.brucelosis  ?? '',
          vac_carbunco:     animal.vacunacion?.carbunco    ?? '',
          vac_clostridial:  animal.vacunacion?.clostridial ?? '',
          vac_ibr:          animal.vacunacion?.ibr         ?? '',
          vac_bvd:          animal.vacunacion?.bvd         ?? '',
        });
        if (animal.lote) setLotes(prev => prev.includes(animal.lote) ? prev : [...prev, animal.lote]);
      })
      .catch(err => {
        setError(err.response?.status === 404 ? 'Animal no encontrado.' : 'No se pudo cargar el animal.');
      })
  ...
  const handleSubmit = async (e) => {
    ...
    setGuardando(true);
    try {
      await actualizarAnimal(id, form);
      setExito(true);
      setTimeout(() => navigate('/animales', { replace: true }), 1500);
    } catch {
      alert('No se pudo guardar. Verificá la conexión e intentá de nuevo.');
    } finally {
      setGuardando(false);
    }
  };
    ...
    <div className="w-full flex flex-col" style={{ gap: '1.75rem' }}>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col" style={{ gap: '1.75rem' }}>
        ...
        <Seccion titulo="Boqueo" icono="🦷">
          <Field label="Cantidad de dientes" nota={!form.fechaNacimiento ? 'Se usará para estimar la edad del animal' : undefined}>
